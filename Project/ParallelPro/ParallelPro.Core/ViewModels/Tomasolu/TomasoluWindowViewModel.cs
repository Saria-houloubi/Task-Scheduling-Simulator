...
namespace Tishreen.ParallelPro.Core
{
    ...
    /// </summary>
    public class TomasoluWindowViewModel : BaseViewModel    
    {
        #region Private Members
        /// <summary>
        /// The name of this class algorithm
        /// </summary>
        private string algoName = "Tomasolu";
        /// <summary>
        ...
        /// </summary>
        public DelegateCommand TomasoluOneCycleCommand { get; private set; }
        /// <summary>
        /// The command to Tomasolu till the end of the algorithm
        /// </summary>
        public DelegateCommand TomasoluTillEndCommand { get; private set; }
        /// <summary>
        ...
        public TomasoluWindowViewModel(List<object> instructionList, Dictionary<FunctionsTypes, int> functionClockCycle)
        {
            ...
            TomasoluOneCycleCommand = new DelegateCommand(StartTomasoluOneStep);
            TomasoluTillEndCommand = new DelegateCommand(StartTomasoluTillTheEnd);
        ...
        #region Tomasulo Algorithm
        /// <summary>
        /// Tomasolus the instructions to a specified clock cycle
        /// </summary>
        private void StartTomasoluTillClockCycle(List<InstructionWithStatusModel> instructions, List<FunctionalUnitWithStatusModel> functionalUnits, List<RegisterResultModel> registers, int toClockCylce)
        {
            while (!Tomasolu(instructions, functionalUnits, registers, toClockCylce)) ;
        }
        /// <summary>
        /// Tomasolu the instruction until it ends
        /// </summary>
        private void StartTomasoluTillTheEnd() { while (!Tomasolu(Instructions, FunctionalUnits, Registers)) ; }
        /// <summary>
        /// only one clock cycle for the algorithm
        /// </summary>
        private void StartTomasoluOneStep() => Tomasolu(Instructions, FunctionalUnits, Registers);
        /// <summary>
        /// The score board algorithm 
        /// </summary>
        private bool Tomasolu(List<InstructionWithStatusModel> instructions, List<FunctionalUnitWithStatusModel> functionalUnits, List<RegisterResultModel> registers, int toClockCycle = -1)
        {
            ...
            foreach (var unit in functionalUnits)
            {
                ...
                if (unit.Function.TryGetValue(instruction.Name, out bool value))
                {
                    ...
                    if (!unit.IsBusy)
                    {
                        ...
                        registerToTarget.InstructionReservedRegiseter.Add(instruction.ID, true);
                        return true;
                    }
                    return false;
                }
        ...
        /// <returns></returns>
        private bool ReadIfApproved(InstructionWithStatusModel instruction, List<FunctionalUnitWithStatusModel> functionalUnits, List<RegisterResultModel> registers)
        {
        ...
        /// </summary>
        private void ExecuteInstrution(InstructionWithStatusModel instruction, List<InstructionWithStatusModel> instructions, List<FunctionalUnitWithStatusModel> functionalUnits, List<RegisterResultModel> registers)
        {
        ...
        private void ClearUnitFunction(FunctionalUnitWithStatusModel unit, List<RegisterResultModel> registers)
        {
            ...
            var registery = registers.SingleOrDefault(reg => reg.WorkingInstructionID == unit.WorkingInstructionID);
            //The registery may be reserved from more than one instruction
            //So if it was then do not free it up just wait for the end of the last instruction
            if (registery != null)
                registery.IsBusy = false;
        ...
        private void SetExamPropertiesAndStoreSolution()
        {
            ...
            StartTomasoluTillClockCycle(InstructionsExamResult, FunctionalUnitsExamResult, RegistersExamResults, ExamClockCycle);
        }
        /// <summary>
